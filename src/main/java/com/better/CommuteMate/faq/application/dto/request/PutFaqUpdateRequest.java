package com.better.CommuteMate.faq.application.dto.request;

import io.swagger.v3.oas.annotations.media.Schema;
import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.NotNull;
import java.util.List;

import java.util.List;

@Schema(description = "FAQ 수정 요청 DTO")
public record PutFaqUpdateRequest(
        @NotBlank
        @Schema(description = "제목", example = "인사관리")
        String title,

        @Schema(description = "민원인 이름", example = "홍길동")
        String complainantName,

        @NotBlank
        @Schema(description = "답변", example = "핸드폰 앱에서 실행 안되고 ...")
        String answer,

        @Schema(description = "비고", example = "없음")
        String etc,

        @NotNull
        @Schema(description = "분류 id 목록 (최대 3개)", example = "[1, 2, 3]")
        List<Long> categoryIds,

        @Schema(description = "내용", example = "학정시 로그인을 하려는데 OTP 관련 메시지가 뜸")
        String content,

        @Schema(description = "이미지 url 리스트", example = "[\"https://kusd.konkuk.ac.kr/uploads/prod/faq/images/3f1c2a9b-1234-4abc-9def-123456789abc.png\"]")
        List<String> imageUrls,

        @Schema(description = "파일 url 리스트", example = "[\"https://kusd.konkuk.ac.kr/uploads/prod/faq/files/3f1c2a9b-1234-4abc-9def-123456789abc.pdf\"]")
        List<String> fileUrls
) {}